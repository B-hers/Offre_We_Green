...
import { searchOdooCatalog, extractWcFromName, OdooCatalogSearchError, type OdooCatalogCandidate } from "../data/odooCatalog";
...
export default function AdminTableEditor({ config }: { config: AdminTableConfig }) {
  ...
  // Upload/suppression de fiche technique PDF (colonnes "file-pdf", decision
  // 50, 15/09/2026) : ecrit directement en Storage + DB des le choix du
  // fichier, en dehors du circuit "Enregistrer" habituel (edited/jsonText) --
  // l'upload est deja une ecriture distante, pas la peine de la faire
  // dependre d'un second clic. Statut affiche par ligne.
  const [fileStatus, setFileStatus] = useState<Record<string, string | null>>({});

  // Recherche/import Odoo -> catalogue (decision 50, 15/09/2026, config
  // odooImportable) : pre-remplit la ligne "Ajouter" a partir d'un article
  // Odoo, l'utilisateur complete/verifie puis cree normalement via
  // createRow -- qui, si un candidat est actif, ajoute en plus une entree
  // product_odoo_links pour tracer le lien (jamais d'ecriture Odoo).
  const [odooQuery, setOdooQuery] = useState("");
  const [odooSearching, setOdooSearching] = useState(false);
  const [odooError, setOdooError] = useState<string | null>(null);
  const [odooCandidates, setOdooCandidates] = useState<OdooCatalogCandidate[]>([]);
  const [odooSelectedCandidate, setOdooSelectedCandidate] = useState<OdooCatalogCandidate | null>(null);

  async function handleOdooSearch() {
    if (!odooQuery.trim()) return;
    setOdooSearching(true);
    setOdooError(null);
    try {
      const results = await searchOdooCatalog(odooQuery.trim());
      setOdooCandidates(results);
    } catch (e) {
      setOdooCandidates([]);
      setOdooError(e instanceof OdooCatalogSearchError ? e.message : "Erreur inattendue lors de la recherche Odoo.");
    } finally {
      setOdooSearching(false);
    }
  }

  function handleOdooPrefill(candidate: OdooCatalogCandidate) {
    setOdooSelectedCandidate(candidate);
    setNewDraft((prev) => ({
      ...prev,
      model_name: candidate.nom ?? prev.model_name,
      unit_cost: candidate.coutStandard || prev.unit_cost,
      power_w: extractWcFromName(candidate.nom) ?? prev.power_w,
    }));
  }
  ...
  async function handleFileUpload(col: AdminColumn, row: Row, file: File) {
    if (!supabase || !col.file) return;
    if (file.type !== "application/pdf") {
      setFileStatus((prev) => ({ ...prev, [row.id]: "Le fichier doit etre un PDF." }));
      return;
    }
    setFileStatus((prev) => ({ ...prev, [row.id]: "Envoi en cours..." }));
    try {
      const safeName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, "_");
      const path = `${row.id}/${Date.now()}-${safeName}`;
      const { error: uploadError } = await supabase.storage
        .from(col.file.bucket)
        .upload(path, file, { contentType: "application/pdf", upsert: false });
      if (uploadError) throw uploadError;
      const previousPath = row[col.key] as string | null | undefined;
      const patch: Row = { [col.key]: path, [col.file.filenameKey]: file.name };
      const { error: updateError } = await supabase.from(config.table).update(patch).eq("id", row.id);
      if (updateError) throw updateError;
      if (previousPath) await supabase.storage.from(col.file.bucket).remove([previousPath]);
      setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, ...patch } : r)));
      setFileStatus((prev) => ({ ...prev, [row.id]: null }));
    } catch (e) {
      setFileStatus((prev) => ({ ...prev, [row.id]: e instanceof Error ? e.message : String(e) }));
    }
  }

  async function handleFileRemove(col: AdminColumn, row: Row) {
    if (!supabase || !col.file) return;
    const path = row[col.key] as string | null | undefined;
    setFileStatus((prev) => ({ ...prev, [row.id]: "Suppression..." }));
    try {
      const patch: Row = { [col.key]: null, [col.file.filenameKey]: null };
      const { error: updateError } = await supabase.from(config.table).update(patch).eq("id", row.id);
      if (updateError) throw updateError;
      if (path) await supabase.storage.from(col.file.bucket).remove([path]);
      setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, ...patch } : r)));
      setFileStatus((prev) => ({ ...prev, [row.id]: null }));
    } catch (e) {
      setFileStatus((prev) => ({ ...prev, [row.id]: e instanceof Error ? e.message : String(e) }));
  ...
  function renderInput(col: AdminColumn, row: Row | null, isNew: boolean) {
    ...
    if (col.type === "file-pdf") {
      if (isNew || !row) {
        return (
          <span className="wg-muted" style={{ fontSize: 12 }}>
            Enregistrez d'abord la ligne
          </span>
        );
      }
      if (!col.file) return null;
      const path = (row[col.key] as string | null) ?? null;
      const filename = (row[col.file.filenameKey] as string | null) ?? null;
      const status = fileStatus[row.id];
      const publicUrl = path && supabase ? supabase.storage.from(col.file.bucket).getPublicUrl(path).data.publicUrl : null;
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 160 }}>
          {publicUrl && (
            <a href={publicUrl} target="_blank" rel="noreferrer" style={{ fontSize: 12 }}>
              {filename ?? "Voir la fiche (PDF)"}
            </a>
          )}
          <input
            type="file"
            ...
            onChange={(e) => {
              ...
              if (file) handleFileUpload(col, row, file);
              e.target.value = "";
            }}
          />
          {publicUrl && (
            <button type="button" className="wg-btn-link" style={{ fontSize: 11, textAlign: "left" }} onClick={() => handleFileRemove(col, row)}>
              Retirer la fiche
            </button>
          )}
          {status && (
            <span className="wg-muted" style={{ fontSize: 11 }}>
              {status}
            </span>
          )}
        </div>
  ...
  async function createRow() {
    ...
    try {
      ...
      if (error) throw error;
      // Trace le lien vers l'article Odoo dont la ligne a ete pre-remplie
      // (decision 50) : jamais une ecriture Odoo, uniquement la table
      // product_odoo_links deja prevue a cet effet (decision anterieure,
      // restee vide jusqu'ici faute d'un import cote UI). Une erreur ici
      // n'annule pas la creation du produit, deja actee -- juste signalee.
      if (config.odooImportable && odooSelectedCandidate && config.table === "products") {
        const { error: linkError } = await supabase.from("product_odoo_links").insert({
          product_id: data.id,
          odoo_product_id: odooSelectedCandidate.odooProductId,
          is_primary: true,
        });
        if (linkError) {
          setCreateError(`Produit cree, mais le lien Odoo n'a pas pu etre enregistre : ${linkError.message}`);
        }
      }
  ...
  return (
    <div>
      {config.odooImportable && (
        <div className="wg-panel wg-data-card" style={{ marginBottom: 12 }}>
          <h2>Importer depuis Odoo</h2>
          <p className="wg-muted" style={{ marginTop: 0 }}>
            Recherche par nom ou reference dans le catalogue produits Odoo (lecture seule -- rien n'est ecrit sur Odoo). Pre-remplit
            la ligne "Ajouter" ci-dessous : verifiez et completez (marque, puissance, marge de securite...) avant de cliquer sur
            "Ajouter".
          </p>
          <div className="wg-inline-group" style={{ marginBottom: 8 }}>
            <label className="wg-field">
              Nom ou reference Odoo
              <input
                type="text"
                value={odooQuery}
                onChange={(e) => setOdooQuery(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleOdooSearch()}
                placeholder="Ex : Jinko Tiger Neo, ou reference interne"
              />
            </label>
            <button type="button" className="wg-btn-primary" disabled={odooSearching || !odooQuery.trim()} onClick={handleOdooSearch}>
              {odooSearching ? "Recherche..." : "Rechercher"}
            </button>
          </div>
          {odooError && (
            <p className="wg-muted" style={{ color: "var(--wg-danger, #b3261e)" }}>
              {odooError}
            </p>
          )}
          {odooCandidates.length > 0 && (
            <table className="wg-sheet" style={{ marginTop: 6 }}>
              ...
              <tbody>
                {odooCandidates.map((c) => (
                  <tr key={c.odooProductId} className={odooSelectedCandidate?.odooProductId === c.odooProductId ? "wg-row-edited" : undefined}>
                    <td>{c.nom ?? "(sans nom)"}</td>
                    <td>{c.reference ?? "-"}</td>
                    <td className="wg-num">{c.coutStandard.toFixed(2)} EUR</td>
                    <td className="wg-num">{c.prixVente.toFixed(2)} EUR</td>
                    <td>
                      <button type="button" className="wg-btn-primary" onClick={() => handleOdooPrefill(c)}>
                        Pre-remplir "Ajouter"
                      </button>
                    </td>
                  </tr>
          ...
          {odooSelectedCandidate && (
            <p className="wg-muted" style={{ marginTop: 6 }}>
              Ligne "Ajouter" pre-remplie depuis "{odooSelectedCandidate.nom}" -- completez les champs manquants puis cliquez sur
              "Ajouter" tout en bas du tableau. Un lien vers cet article Odoo sera conserve automatiquement.
            </p>
          )}
        </div>
      )}
